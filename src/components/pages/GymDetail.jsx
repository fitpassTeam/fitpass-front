...
function GymDetail() {
  ...
  return (
    <div className="max-w-5xl mx-auto mt-10 p-6 bg-white rounded-2xl shadow-xl flex flex-col">
      {/* 상단: 체육관 소개 영역 */}
      <div className="flex flex-col md:flex-row gap-8 bg-white rounded-2xl shadow p-6 mb-8">
        {/* 왼쪽: 사진 슬라이드 */}
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-center">{tab === 'pass' ? '이용권 및 PT' : '게시글'}</h1>
        <button
          className="px-4 py-2 bg-gray-200 rounded-lg font-semibold text-gray-700 hover:bg-blue-100 transition"
          onClick={() => setTab(tab === 'pass' ? 'post' : 'pass')}
        >
          {tab === 'pass' ? '게시글 보기' : '이용권 및 PT 보기'}
        </button>
      </div>
      {tab === 'pass' ? (
